...
(async () => {
  ...
  const checks = [{ l:'Price', s: T.price, a: integFields.PRICE }, { l:'Fwd P/E', s: F.fwdPE, a: integFields.FWDPE }, { l:'Rev Gr', s: F.revGr, a: integFields.REVGR }];
...
<div id="overview" class="tab-content active">
  ...
  ${altData.length ? `
  <div class="panel">
    <h2>📡 Alternative Data Signals</h2>
    <div class="grid g3">
        ${altData.map(a => `<div style="padding:16px; background:rgba(255,255,255,0.03); border-radius:12px; border:1px solid var(--border);"><span style="font-size:20px; display:block; margin-bottom:8px;">🛰️</span><p class="story-text" style="margin:0; font-size:14px;">${a}</p></div>`).join('')}
    </div>
  </div>` : ''}
