 ...
 *
 * Fixes the core timing flaw: the old screener's "COILED SPRING" required a
 * volume surge, i.e. it detected the spring AFTER release. A coil and its
 * trigger are different events:
 *
 *   COILING   — energy storing. Vol compression + supply dry-up + tight base
 *               just under a pivot. NOT a buy. Goes on the watchlist with an
 *               alert level. (catches it "not too early")
 *   TRIGGERED — TODAY the pivot broke on a volume thrust with a strong close,
 *               and price is still inside the buy zone (≤3% / ≤1×ATR above
 *               pivot). THIS is the entry. (catches it "right on time")
 *   EXTENDED  — broke out but >5% or >1.5×ATR past pivot. Chasing kills R:R.
 *               Wait for a pivot retest. (refuses it "not too late")
 *   REVERSAL  — washed-out + first higher-low + reclaim of 20d MA on volume.
 *
 * Pure functions over quotes [{open,high,low,close,volume}], no Yahoo coupling.
 */
...
function compressionScore(quotes) {
  ...
  const dry = volDryUp(quotes);                                // supply exhausted?
  if (dry != null && dry < 0.8) { score += 15; parts.volDryUp = dry; }
  else if (dry != null && dry < 1.0) { score += 7; parts.volDryUp = dry; }

  const cs = contractionSequence(quotes);                      // VCP-style tightening
  if (cs && cs.contracting) { score += 15; parts.vcp = cs.ranges; }

  const udv = upDownVolRatio(quotes);                          // quiet accumulation
...
/** Pivot = highest close of prior `lookback` days EXCLUDING today. */
...
/**
 * Classify. Returns { state, score, entry, stop, t1, t2, rr, pivot, detail }.
 * opts.maxStopPct caps stop distance (default 8%).
 */
function classifySetup(quotes, opts = {}) {
  ...
  const comp = compressionScore(quotes.slice(0, -1)); // compression measured on the BASE, not the breakout bar
  ...
  // ---- stop construction (fixes swing-low bug: clamp distance) ----
  const swingLow = Math.min(...quotes.slice(-10).map(q => q.low));
  let stop = Math.max(swingLow, price - 2 * atr);          // tighter of the two valid stops
  stop = Math.max(stop, price * (1 - maxStopPct));         // never risk > maxStopPct
  if (stop >= price) stop = price - 1.5 * atr;             // degenerate guard

  // ---- measured-move target: base depth projected from pivot (no analyst fiction) ----
  const baseLow = Math.min(...quotes.slice(-45).map(q => q.low));
  const baseDepth = pivot - baseLow;
  const t1 = +Math.max(pivot + baseDepth * 0.5, price + 2 * (price - stop)).toFixed(2); // ≥2R floor from ENTRY
  ...
  // ── COILING: watchlist with alert level ──
