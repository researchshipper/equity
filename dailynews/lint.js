...
// marketdata.factCheck compares report mood/macro numbers against live data.
// Optional require so the linter still runs if the file is absent.
let marketdata = null;
try { marketdata = require('./marketdata.js'); } catch { /* optional */ }
  ...
  const checks = [
    {
      ...
      extras: (html) => {
        ...
        // E013: Dual 7d + 30d insider tables (deterministic requirement)
        if (!/7d \(aggressive recent\) \+ 30d \(smoother primary\)/.test(html))
          issues.push({ code:'E013', ...errs('marketbeat_report.html missing dual 7d+30d insider header (must contain "7d (aggressive recent) + 30d (smoother primary)").', 'marketbeat_report.html') });
        if (!/Last 7 days — aggressive latest buys/.test(html) || !/Last 30 days — smoother aggregate \(primary filter\)/.test(html))
          issues.push({ code:'E013', ...errs('marketbeat_report.html missing one or both insider tables (need separate "Last 7 days" and "Last 30 days" sections).', 'marketbeat_report.html') });
        // E014: Rich macro analytical style (key phrases from reference)
        const macroText = (html.match(/class="macro"[\s\S]{0,2000}/) || [''])[0];
        if (!/JOLTS|2-yr high|re-fired|hawkish tilt|Job Openings.*est|ADP.*vs .*fcst/.test(macroText))
          issues.push({ code:'E014', ...errs('marketbeat_report.html macro tiles missing rich analytical style (need JOLTS "2-yr high" / "re-fired", ADP "vs fcst" + hawkish tilt style implications). See PROMPT.md deterministic section.', 'marketbeat_report.html') });
        return issues;
      },
    },
    {
    ...
    {
      file: 'scoreboard_7d.html', codeBase: 'E8', required: false, // standalone: no scoreboard history required
    },
  ];
...
// ─── run ────────────────────────────────────────────────────────────────────
// ─── E012 number fact-check (report vs live marketdata.json) ────────────────
// The strongest guard against hand-typed/hallucinated numbers: every mood cell
// that matches a live instrument must be within tolerance of the real print.
// Only fires when marketdata.json exists AND was fetched successfully — if the
// data sources were blocked at fetch time we can't verify, so we note it softly.
function checkFactCheck(dir, report, out){
  const mdPath = path.join(dir, 'marketdata.json');
  if (!marketdata || !marketdata.factCheck){
    out.push({ code:'W212', ...warn('marketdata.js not available — number fact-check skipped. Add it to anchor mood/macro to real prints.', 'marketdata.json') });
    return;
  }
  if (!fs.existsSync(mdPath)){
    out.push({ code:'W212', ...warn('marketdata.json not found — run `node marketdata.js` to enable number fact-check against live data.', 'marketdata.json') });
    return;
  }
  let market;
  try { market = JSON.parse(fs.readFileSync(mdPath,'utf8')); }
  catch { out.push({ code:'E012', ...errs('marketdata.json is not valid JSON — re-run `node marketdata.js`.', 'marketdata.json') }); return; }

  if (market.status !== 'ok'){
    out.push({ code:'W213', ...warn(`marketdata.json status="${market.status}" (sources blocked at fetch) — numbers could NOT be fact-checked this run. Verify mood/macro by hand.`, 'marketdata.json') });
    return;
  }
  // staleness: data should be from the report date (or one day off for overnight)
  if (market.asof && report.date && market.asof < report.date){
    out.push({ code:'W214', ...warn(`marketdata.json is from ${market.asof} but report.date is ${report.date} — refresh with \`node marketdata.js\` before trusting the fact-check.`, 'marketdata.json') });
  }
  const issues = marketdata.factCheck(report, market);
  for (const msg of issues){
    out.push({ code:'E012', ...errs(`number off vs live data — ${msg}`, 'mood') });
  }
}

// ─── timestamp + commentary quality ────────────────────────────────────────
function checkStampAndCommentary(report, out){
  // report.time lets multiple same-day runs be told apart (user request).
  if (!len(report.time))
    out.push({ code:'W215', ...warn('report.time missing — add a "time" field (e.g. "9:35a ET") so multiple same-day runs are distinguishable in the title/header.', 'time') });
  if (!len(report.generatedAt))
    out.push({ code:'W216', ...warn('report.generatedAt missing — set an ISO timestamp when you finalize the report.', 'generatedAt') });

  // marketCommentary: holistic multi-view "why is the tape doing this" section.
  const mc = report.marketCommentary;
  if (!mc || (!len(mc.summary) && !mc.views)){
    out.push({ code:'W217', ...warn('marketCommentary missing — add the holistic "why is the sector/tape up/down today" section (summary + bull/bear/base views).', 'marketCommentary') });
  } else {
    const v = mc.views || {};
    const filledViews = ['bull','bear','base'].filter(k => len(v[k])).length;
    if (filledViews < 2)
      out.push({ code:'W217', ...warn(`marketCommentary.views has only ${filledViews}/3 lenses — fill at least bull + bear + base for genuine multi-view thinking.`, 'marketCommentary') });
    if (len(mc.summary) && len(mc.summary) < 120)
      out.push({ code:'W218', ...warn(`marketCommentary.summary is thin (${len(mc.summary)} chars) — synthesize what moved, how much, and the dominant driver with real numbers.`, 'marketCommentary') });
    if (!(mc.drivers||[]).length)
      out.push({ code:'W219', ...warn('marketCommentary.drivers[] empty — list the 3-6 forces pushing the tape.', 'marketCommentary') });
  }
}
...
function lint(report, opts = {}){
  ...
  checkStampAndCommentary(report, findings);
  ...
  if (!opts.skipCompanions){
    ...
    checkFactCheck(dir, report, findings);
  }
...
## Number accuracy (E012 / W212-W214) — fact-check is a hard gate

- Run \`node marketdata.js\` first; it writes \`marketdata.json\` with REAL prints.
- Every \`mood[]\` value that names an index/future/commodity/rate/crypto/FX must
  match the live print within tolerance. If E012 fires, **copy the real number**
  from marketdata.json (or run \`node marketdata.js --print\` for a paste-ready
  mood[] seed) — do NOT hand-type levels from memory.
- Same discipline for \`macro\` numbers (CPI/NFP forecasts, 10Y, oil): cite real,
  verified figures. If you can't verify, say "est." rather than inventing precision.

## Timestamp (W215/W216)

- Set \`time\` (e.g. "9:35a ET") and \`generatedAt\` (ISO) so multiple same-day runs
  are distinguishable in the title + header.

## Market commentary (W217-W219) — multi-view thinking

- Fill \`marketCommentary\` with a holistic read of WHY the sector/tape is up/down:
  \`summary\` (with numbers), \`drivers[]\`, and \`views\` {bull, base, bear}, plus
  \`whatWouldChangeMind[]\`. Argue more than one angle — that's the point.
`;
